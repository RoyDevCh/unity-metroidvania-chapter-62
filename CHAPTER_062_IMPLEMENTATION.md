...
- 043-047：`DemoBootstrap` 在 Play Mode 生成平台训练场，并加载三层像素背景。学习 Tile Palette、Tilemap Collider、Cinemachine 和视差时，可按文档将这些运行时平台替换为编辑器资源；核心控制器不需要改。
...
场景中的角色和敌人已使用 `Assets/Resources/Chapter62Art` 的像素素材；为保持终点严格停在第 62 章，目前使用单帧显示，未加入第 063 章之后的 Skill Manager、克隆和飞剑。
